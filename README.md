...
| | 誤差 | 対象 |
|---|---|---|
| 本データ `town`（実測） | **中央値0.52 km**（90%ile 2.88km） | 44,132行（62.0%） |
| 本データ `oaza`（実測） | **中央値0.30 km** | 5,678行（8.0%） |
| **2008年版の公式座標** | **RMS約0.7 km / 最大1.2 km** | 57,847件 |
| 本データ `city`（実測） | **中央値3.14 km** | 15,063行（21.2%） |

中央値だけを見れば町字レベルは2008年版より良いのですが、**実測すると90%ileで2.88kmまで伸びる**ため、
「本データの方が精度が高い」と単純には言えません。市区町村レベルの21.2%は2008年版の方が明確に優れています。

### 2008/2009年版の入手可能性（調査済み・入手できず）

G-Spaceは「2009年8月にダウンロードした」データで57,847件をプロットしたとしています。当時の配布物を
Wayback Machine で年代別に追跡しました。当時の所管は**国土交通省 土地・水資源局 国土調査課**、
資料名は**水基本調査（地下水調査）／全国地下水資料台帳**（印刷物は「全国地下水（深井戸）資料台帳」）で、
ホストは `tochi.mlit.go.jp/tockok/`、データは `/tockok/tochimizu/F9/data/` にありました。

| 時期 | 配布物 | X/Y列 |
|---|---|---|
| 2007-11 / 2008-01 | 都道府県別zip 47本 ＋ **地域別zip 8本**（`hokkaido.zip` など。現存しない） | 実ファイルがアーカイブされておらず未確認 |
| 2011-09 | 都道府県別zip（京都・岡山・佐賀が保存） | **実物を開いて確認 → 全行空** |
| 2013-01 | 都道府県別zip（`nrb-www.mlit.go.jp` へ移行後） | **実物を開いて確認 → 全行空** |

2007〜2010年のデータ構造ページには `X 経度` `Y 緯度` が普通のフィールドとして載っており、
現在ある「経緯度は掲載しておりません」の注記はまだありません。座標が入っていた時期が実在した可能性は
残りますが、**現存する実ファイルはすべて空**でした。

そのほか、国土数値情報のデータ一覧・旧版（gmlold）・国土情報ウェブマッピングのレイヤ定義・GitHub全文検索も
当たりましたが、井戸の点データはありません。G-Space は月額5,500円の閲覧サービスで、一括ダウンロードや
再配布の可否は示されていません。

**結論として、座標付き2008年版とのハイブリッドは実行手段がありません。** 代わりに位置図PDFから
378点の真位置を復元し、精度の実測に用いています（前述）。
