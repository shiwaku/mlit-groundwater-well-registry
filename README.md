...
| 災害時協力井戸（各自治体） | 21データセット | あり | 全国横断の整備なし。**深度を持つのは船橋市だけ**（下記） |

### 自治体のオープンデータを全走査した結果

横断カタログ3つ（[G空間情報センター](https://front.geospatial.jp/) /
[BODIK ODCS](https://odcs.bodik.jp/) /
[東京都オープンデータカタログ](https://catalog.data.metro.tokyo.lg.jp/)）と
熊本県のデータ連携基盤を、9語（`井戸` `湧水` `揚水` `さく井` `深井戸` `水源井` `観測井`
`地下水位` `水道水源`）で検索しました。井戸系のデータセットは**全国で21件**しかなく、
ほぼ全部が防災用井戸・災害時協力井戸です。取得と判定は
[`scripts/15_fetch_municipal_wells.py`](scripts/15_fetch_municipal_wells.py) が行います
（`--search` でカタログ検索だけを再実行できます）。

正解データにするには **位置（番地住所か緯度経度）と深度の両方**が要ります。深度が無いと
F9のどの行と同じ井戸なのかを確定できず、距離を測っても精度の実測値になりません。

| 自治体・データ名 | 件数 | 座標 | 番地 | 深度 | F9 2003（うち`unique`） |
|---|---:|:-:|:-:|:-:|---:|
| **船橋市 防災用井戸一覧** | 28 | − | ○ | **○** | 134（46）**←正解データに採用** |
| 千葉市 非常用井戸等 | 222 | ○ | ○ | − | 317（176） |
| 練馬区 学校防災井戸 | 102 | − | ○ | − | 104（23） |
| 東村山市 防災用井戸 | 94 | ○ | ○ | − | 54（3） |
| 相模原市 災害時協力井戸一覧 | 81 | ○ | ○ | − | 121（43） |
| 熊本県 地下水位観測地点 | 65 | ○ | − | − | 280（136） |
| 流山市 災害用井戸設置場所 | 32 | ○ | ○ | − | 52（12） |
| 練馬区 防災井戸 | 23 | − | ○ | − | （上に含む） |
| 市川市 防災用井戸 | 14 | ○ | ○ | − | 69（17） |

**深度を持つのは船橋市だけでした。** 「深度と番地住所の両方を公開している自治体だけが
正解になる」という条件が、カタログ全走査でも変わらないことが確認できています。

対象外にしたものもあります。**那覇市「井戸・湧水台帳」**（170点・シェープ）は
`末吉大樋川` `チョンチョンガー` など沖縄の湧水・カーで、F9（30m以深の深井戸）とは母集団が違います
（F9側の那覇市は5点のみ）。**名古屋市の汚染井戸調査**は DOCX の報告文書で構造化データではなく、
**東京都建設局の地盤沈下・地下水位観測記録**は観測井ごとの時系列CSVだけで位置・深度の一覧がありません。

なお `odcs.bodik.jp` は `data.bodik.jp` の CKAN を表示するポータルで、実体は同じです。
市川市のCSVはヘッダが2段でデータ行と列がずれているため、列名からは住所列を特定できません
（判定スクリプトは値の形からも住所列を拾って補っています）。
...
| 由来 | 点数 | 内訳 |
|---|---:|---|
| 原本の座標 | 503 | 千葉市222・東村山市94・相模原市81・熊本県65・流山市28・市川市13 |
...
CSVごとに列名がばらばらなので、位置・名称・住所・深度は列名の手がかりで拾っています。
住所列だけは列名で選ぶと練馬区の `所在地_全国地方公共団体コード`（値は `131202`）を拾ってしまうため、
住所らしい値がいちばん多い列を採る方式にしています。
...
2003年版の **57,847点を位置精度で色分けして表示する** 地図ビューワを同梱しています。
...
**既定は位置精度がもっとも高い15,452点です。** 地図を開いた人はまず「点がある＝そこに井戸がある」と
読むので、既定をそのまま信用してよい点に揃えました。独立した2つの条件を両方満たすものだけを出しています。

| 既定の条件 | 落ちるもの |
|---|---|
| `POS_QUALITY == "unique"`（実位置・単独の座標） | `fallback` 16,081点（市区町村代表点）と `site` 5,351点（同一サイトへの相乗り） |
| `ADR_DIST_KM` が測れている（町名と突合できた） | 33,492点。うち `unique` は20,963点 |

2つ目は**座標の質ではなく裏取りができたかどうか**です。突合できなかったのは
住所マスタに該当町字が無い（18,348点）、同名町字が県内に散在して一箇所に定まらない（10,316点）、
原本に町名が無い（4,828点）ためで、`幸町` のような点は座標が誤っている証拠があるわけではありません。
そのぶん既定は厳しめで、**外すと36,415点に戻ります**（絞り込みのチェックボックス、または凡例の淡い行から）。
この既定は5つの色分けすべてに共通で効きます。

表示に戻したときも消さずに区別します。どの主題で色分けしていても
**`fallback` だけは塗らずに輪郭のみ** で描くので、色を追わなくても
「この点の位置は信用できない」ことが分かります。
...
| 所在地からの距離 | `ADR_DIST_KM`。自分の町名の位置からどれだけ離れているか。5段階＋町名と突合できず |
| 掘削深度 | `DEP`。50 / 100 / 150 / 250m 区切り |
| 調査年度 | `NEN`。年代区切り＋不明 |
| 地下水使用目的 | `USE`。生活用・都市用・工業用・農業用・その他・未利用 |

凡例の各行はクリックでその区分だけの表示に切り替わります。ほかに位置精度のチェックボックス（`POS_QUALITY`
の3区分＋町名と突合できたか）、`ADR_DIST_KM` の上限、掘削深度の下限で絞り込めます。位置精度で隠している区分は
位置精度の凡例では淡く表示され、その行を押すと表示に戻ります（押しても何も起きない行を作らないため）。

突合できたかのチェックボックスは、距離の絞り込みではなく **位置精度** の側に置いています。
「距離を測れたか」は距離の値ではなく位置を裏取りできたかどうかの話で、`POS_QUALITY` と同じ軸だからです。
UI では「判定不能」ではなく「町名と突合できなかった点」と書いています（前者だと位置が怪しい点に読めるため）。
...
背景は淡色（地理院 最適化ベクトルタイル）／全国最新写真から選べます。
...
# 自治体が公開する井戸データを調べるとき
python3 scripts/15_fetch_municipal_wells.py            # 取得して「突合に使えるか」を判定
python3 scripts/15_fetch_municipal_wells.py --geojson  # -> output/municipal_wells.geojson（656点）
python3 scripts/15_fetch_municipal_wells.py --search   # カタログ検索だけ再実行（新規公開の確認）
