...
| `output/f9_wells_2003.csv` | **座標が入っていた2003年版**（配布終了。ジオコーディング不要） | 57,847 × 48 | 17MB |
| `output/f9_wells_2003.parquet` | 同上の GeoParquet | 57,847 × 49 | 5.6MB |

CSVの文字コードは **UTF-8（BOMなし）**、改行は CRLF です。

> [!TIP]
> **座標の精度だけが要るなら `f9_wells_2003` を見てください。** 2003年版は原本に緯度経度が入っており、
> 粒度は日本測地系の秒単位（約30m）です。ジオコーディングした現行版（数百m〜数km）より1〜2桁良い一方、
> 収録は57,847件で年次も2003年までなので、件数と新しさが要るなら現行版になります。詳しくは
...
量子化だけを見れば **RMS約12m・最大約20m** です。

「記録された秒の値そのものが正しいか」を測る独立した正解データはありませんが、傍証を2つ取りました。

| 検証 | 結果 |
|---|---|
| 行政区域（N03）内判定（北海道・東京・石川・愛知・京都・福岡の17,038点） | **99.72%** が自県の区域内 |
| 現行版のジオコーディング結果との距離（1対1で突合できた42,041行） | 中央値 **0.70km**（`town` 0.62 / `oaza` 0.77 / `city` 1.31km） |

2つ目は2003年版側の誤差ではなく、**ほぼジオコーディング側の誤差**です。位置図PDFの真位置378点で実測した
ジオコーディングの誤差（`town` 中央値0.52km、`city` 中央値3.14km）と整合しており、2003年版が大きく外れて
いないことを示しています。**数十メートル級で、ジオコーディング（数百m〜数km）より1〜2桁良い**というのが結論です。

ただし**残り0.28%（全体では160件前後にあたる）には実際に誤りが含まれます**。県外に出た点について
区域からの距離を測ったところ、東京都では19点のうち6点が1km超（最大4.8km）、京都府では3点のうち1点が
6.9kmでした。海岸線の描画差では説明できない値なので、原本の秒の記録そのものが誤っている行がある、
ということです。**個々の点を無条件に信用せず、用途に応じて自県・自市区町村に落ちるかの確認を挟んでください。**
...
`scripts/12_shp2003_to_dataset.py` が8本をまとめ、`MUNI_CD`（`PREF`+`CITY` の5桁）と
`TEMP_KIND`（次項）を足して出力します。原本の欠陥を2つそのまま引き継いでいます。
