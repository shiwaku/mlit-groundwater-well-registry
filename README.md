...
**既定では `unique`（実位置・単独の座標）36,415点だけを表示します。** 残る21,432点は
`fallback`（16,081点・市区町村代表点）と `site`（5,351点・同一サイトへの相乗り）で、
地図を開いた人はまず「点がある＝そこに井戸がある」と読みます。既定を
そのまま信用してよい点に揃え、残りは絞り込みのチェックボックス（または凡例の淡い行）で
足してもらう形にしています。この既定は5つの色分けすべてに共通で効きます。
...
`ADR_DIST_KM` の上限、掘削深度の下限で絞り込めます。位置精度で隠している区分は
